...
@WebServlet(urlPatterns = {"/chinese-volunteers"})
public class ChineseVolunteersServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            List<CstDepartment> chineseDeps = DepartmentFilterUtil.filterChinese(allDeps);
            String q = req.getParameter("q");
            if (q != null && !q.trim().isEmpty()) {
                String qNorm = q.trim().toLowerCase();
                chineseDeps.removeIf(d -> d.getName() == null || !d.getName().toLowerCase().contains(qNorm));
            ...
            int total = chineseDeps.size();
            int totalPages = (int) Math.ceil(total / (double) size);
            if (page > totalPages && totalPages > 0) page = totalPages;
            int from = (page - 1) * size;
            int to = Math.min(from + size, total);
            List<CstDepartment> paged = chineseDeps.subList(from, to);
            ...
            if (chineseDeps.isEmpty()) {
                req.setAttribute("emptyChinese", Boolean.TRUE);
            }
            ...
            com.smartcalendar.utils.AnalyticsLog.log(userId, "/chinese-volunteers", queryParams);
            req.setAttribute("members", members);
            req.getRequestDispatcher("chinese-volunteers.jsp").forward(req, resp);
