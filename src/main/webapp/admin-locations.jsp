                ...
                <details class="loc-acc">
                    ...
                    <div class="acc-body">
                        <form method="post" action="admin-locations" class="inline-edit" style="margin-bottom:8px">
                            ...
                            <div class="form-actions" style="display:flex;flex-direction:column;align-items:center;gap:10px;margin-top:12px">
                                <label style="display:inline-flex;align-items:center;gap:8px;font-size:.8rem;color:#374151"><input type="checkbox" name="active" <%= l.isActive()?"checked":"" %> /> Active</label>
                                <div style="display:flex;gap:10px;justify-content:center">
                                    <button type="submit" class="btn btn-primary btn-sm" style="min-inline-size:100px">Save</button>
                                    <a class="btn btn-outline btn-sm" style="min-inline-size:100px" href="<%= l.getMapUrl() %>" target="_blank" rel="noopener">Open Map</a>
                                </div>
                            </div>
                        </form>
                        <form method="post" action="admin-locations" onsubmit="return confirm('Delete this location?');">
                            ...
                            <input type="hidden" name="locationId" value="<%= l.getLocationId() %>" />
                            <input type="hidden" name="returnCategory" value="<%= currentCategory %>" />
                            <button type="submit" class="btn btn-danger btn-sm" style="min-inline-size:100px">Delete</button>
                        </form>
                    </div>
                </details>
            <% } } %>
